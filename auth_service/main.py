...
#     parsed_data: Dict[str, Any]  # Fixed: was "parsed_date"
...
# # ✅ FIXED: Removed early return, fixed logic
...
# # ✅ FIXED: Complete resume upload endpoint
...
#     file_path = os.path.join(UPLOAD_DIR, filename)  # ✅ Fixed variable name
...
#         # ✅ FIXED: Correct INSERT statement with all required columns
#         cursor.execute("""
#             INSERT INTO resumes (
...
#         connection.commit()  # ✅ FIXED: Moved inside try block
...
# # ✅ FIXED: Corrected stats endpoint
# @app.get("/stats")
...
#     try:
#         cursor.execute(
#             "SELECT id FROM candidate_profile WHERE user_id=%s", (current_user["id"],)
#         )
...
#                         full_name=%s, phone=%s, location=%s, experience_years=%s, linkedin_url=%s, updated_at=NOW()
#                         WHERE user_id=%s
#                     """, (
...
#             profile_id = str(uuid.uuid4())
#             cursor.execute("""
#                 INSERT INTO candidate_profile (id, user_id, full_name, phone, location, experience_years, linkedin_url, created_at, updated_at)
#                 VALUES (%s, %s, %s, %s, %s, %s, %s, NOW(), NOW())
#             """, (
#                 profile_id, current_user["id"], profile.full_name, profile.phone, profile.location,
...
async def get_own_profile(current_user: dict = Depends(get_current_user)):
    """Get current candidate's own profile (GET - no request body)"""
    if current_user["role"] != 'candidate':
