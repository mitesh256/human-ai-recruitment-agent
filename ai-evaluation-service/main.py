...
if not GENAI_API_KEY:
    print("Warning: GENAI_API_KEY or GEMINI_API_KEY not found in environment variables")
else:
    genai.configure(api_key=GENAI_API_KEY)

# Fixed: Better model initialization with error handling
try:
    sentence_model = SentenceTransformer('all-MiniLM-L6-v2')
except Exception as e:
...
class AIEvaluator:
    def __init__(self):
        try:
            self.model = genai.GenerativeModel("gemini-1.5-pro") if GENAI_API_KEY else None
        except Exception as e:
    ...
    def calculate_semantic_similarity(self, job_text: str, resume_text: str) -> float:
        ...
        if not sentence_model:
            return 60.0  # Default score if model not available
    ...
    def evaluate_technical_skills(self, required_skills: List[str], candidate_skills: List[str]) -> int:
        ...
        matched_skills = set(required_lower).intersection(set(candidate_lower))
        skill_match_ratio = len(matched_skills) / len(required_skills)
    ...
    def evaluate_experience(self, required_years: int, candidate_years: int) -> int:
        """Evaluate experience level"""
        if candidate_years >= required_years:
            return min(90 + (candidate_years - required_years) * 2, 100)
        else:
            # Penalty for less experience, but not too harsh
            shortage = required_years - candidate_years
            penalty = min(shortage * 15, 60)
    ...
    async def generate_ai_explanation(self, scores: Dict, job_title: str, candidate_name: str) -> str:
        """Generate explanation using Gemini API"""
        if not self.model:
            return f"Candidate shows {scores['overall_score']}% compatibility with the {job_title} position. Technical skills align well with requirements, and experience level is appropriate for the role."
    ...
    async def evaluate_candidate(self, request: CandidateEvaluationRequest) -> Dict:
        ...
        # Education score - improved logic
        education_score = 75
        if candidate.get("education"):
            if isinstance(candidate["education"], dict):
                if candidate["education"].get("degree"):
                    education_score = 85
            elif isinstance(candidate["education"], list) and candidate["education"]:
                education_score = 85
        else:
            education_score = 50
        ...
        # Generate confidence score
        score_variance = max([technical_score, experience_score, cultural_fit_score, education_score]) - min([technical_score, experience_score, cultural_fit_score, education_score])
        confidence = max(95 - (score_variance // 10), 60)
        ...
        return {
            ...
            "timestamp": datetime.now().isoformat(),
            "breakdown": {
                "technical_match": f"{len(set(job_req.get('required_skills', [])).intersection(set(candidate.get('skills', []))))} of {len(job_req.get('required_skills', []))} required skills matched",
                "experience_gap": max(0, job_req.get("required_experience", 0) - candidate.get("experience_years", 0)),
                "key_strengths": candidate.get("skills", [])[:5],
                "recommendations": [
                    "Consider for interview" if overall_score >= 70 else "Requires further evaluation",
                    "Strong technical background" if technical_score >= 70 else "Consider skill assessment",
                    "Good cultural fit" if cultural_fit_score >= 70 else "Review company culture alignment"
                ]
            }
        }

# Fixed: Better token verification with error handling
async def verify_token(token: str = Depends(security)):
    try:
        response = requests.get(
            "http://localhost:8001/profile",
...
async def get_evaluation_explanation(
    ...
    return {
        ...
        "explanation": stored_eval["explanation"],
        "scores": stored_eval["scores"],
        "breakdown": stored_eval.get("breakdown", {}),
        "confidence_factors": [
            "Technical skills alignment with job requirements",
            "Experience level matches expected qualifications", 
            "Resume content indicates cultural fit",
        ...
        "timestamp": stored_eval["timestamp"]
    }

@app.get("/api/v1/ai-evaluation/history")
async def get_evaluation_history(current_user: dict = Depends(verify_token)):
    """Get evaluation history for current user"""
    user_evaluations = [
        eval_data for eval_data in evaluation_storage.values()
        if eval_data.get("user_id") == current_user["user"]["id"]
    ]
...
@app.post("/api/v1/ai-evaluation/batch")
async def evaluate_multiple_candidates(
    requests: List[CandidateEvaluationRequest],
    current_user: dict = Depends(verify_token)
):
    ...
    for req in requests:
        try:
            result = await evaluator.evaluate_candidate(req)
            results.append(result)
        except Exception as e:
            results.append({
                "error": str(e),
                "candidate": req.candidate_profile.get("name", "Unknown")
            })
...
async def health_check():
    return {
        ...
        "service": "ai-evaluation-gemini",
        "port": 8004,
        "model": "gemini-1.5-pro" if GENAI_API_KEY else "fallback-mode",
        "sentence_model": "all-MiniLM-L6-v2" if sentence_model else "unavailable"
    }
